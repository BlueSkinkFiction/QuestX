...
erotica.goDownScript = function(objects) {
  ...
  if (target.hasBodyPart("pussy")) {
    return cmdInteract(parser.specialText.action.find("lick"), char, target,{bodypart: w.pussy})
  }
...
erotica.teabagScript = function(objects) {
  ...
  if (target.hasBodyPart("bollock")) {
    return cmdInteract(parser.specialText.action.find("suck"), char, target, {bodypart:w.bollock})
  }
...
erotica.ACTIONS = [
...

  {
    ...
    extraTests:function(options) {
      //console.log(target)
      options.garment = target.getOuterWearable("crotch")
      if (options.garment) {
        if (options.garment.wearable) return falsemsg("{nv:char:cannot:true} do that while {nv:item:be} is wearing {nm:garment:a}.", options)
        return falsemsg("{nv:char:cannot:true} do that while {nv:item:be} tied to {nm:garment:the} like that.", options)
      }
      options.garment = char.getOuterWearable("crotch")
      if (options.garment) return falsemsg("Not while {nv:char:be} wearing {nm:garment:a}.", options)
      return true
    },
  },
...

  {
    ...
    extraTests:function(options) {
      ...
      if (!options.char.canManipulate(null, "wank")) return world.FAILED;
...

  {
    ...
    extraTests:function(options) {
      ...
      if (!options.char.canManipulate(null, "frig")) return world.FAILED;
...
commands.unshift(new Cmd('SexActionsMy', {
  ...
  script:function(objects) {
    const char = extractChar(this, objects)
    if (!char) return world.FAILED;
    return cmdInteract(objects[0], char, player, {bodypart:objects[2][0], side:objects[1]});
  },
}));
...
commands.unshift(new Cmd("SexActions", {
  ...
  script:function(objects) {
    const char = extractChar(this, objects)
    if (!char) return world.FAILED;
    return cmdInteract(objects[0], char, objects[1][0], {bodypart:objects[3][0], side:objects[2]});
  },
}));
...
function cmdInteract(verb, char, object, options) {
  ...
  if (options.bodypart) {
    ...
    if (verb.getDefaultBodyPart) {
      options.bodypart = bodypart = w[verb.getDefaultBodyPart(char, object)]
    }
    else {
      log(options)
      log(verb)
  ...
  if (!char.canManipulate(object, verb.name)) return world.FAILED;
  ...
  respond(options, erotica.defaultResponses, respondCompleted);
  return world.SUCCESS;
}
...
erotica.wankScript = function(objects) {
  ...
  if (target.hasBodyPart("cock") && !verb.startsWith("jill") && !verb.startsWith("flick")) {
    return cmdInteract(parser.specialText.action.find("wank"), char, target, {bodypart:w.cock, altName:verb})
  }
  if (target.hasBodyPart("pussy") && !verb.startsWith("jerk") && !verb.startsWith("jack") && !verb.startsWith("rub") && !verb.endsWith("tug")) {
    return cmdInteract(parser.specialText.action.find("frig"), char, target, {bodypart:w.pussy, altName:verb})
  }
...
commands.unshift(new Cmd("LookAtBP", {
  ...
  script:function(objects) {
    ...
    if (item.isBodyPart) {
      msg(target.describeBodyPart(item.name), {item:target, bodyPart:item})
    }
    else {
      printOrRun(player, item, "examine", {multi:false, match:false, verb:objects[0]});
    }
    return world.SUCCESS;
  },
}));
...
// canComeOver assumes char is either stood, kneeling or stood on something
  ...
  
  {
    ...
    getAssumePostureDescription:function(char, previous) {
      if (previous === 'reclining') {
        ...
        return "{nv:char:lie:true} back on the " + (w[this.loc].postureSurface || "floor") + ", face down."
      }
    },
  },
...
function cmdUndress(char) {
  ...
  if (!char.canManipulate(garment, "Remove")) return world.FAILED
...
function cmdRemoveGarment(char, target, garment, strength) {
  ...
  const targetNoChoice = target.restraint && !w[target.restraint].canManipulate
...
function cmdRestrain (char, target, restraint) {
  ...
  if (held.length > 0) {
    ...
    msg("{nv:char:take:true} " + formatList(held, {article:DEFINITE, lastJoiner:lang.list_and}) + " from {nm:item:the} and {cj:char:drop} " + it_them + " on the ground.", options)
