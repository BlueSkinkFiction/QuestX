...
https://github.com/BlueSkinkFiction/QuestX/wiki
...
test.tests = function() {
  ...
  w.briefsblack.loc = "lounge"
  world.update()
  test.assertCmd("get bikini", "You take the black bikini.")  // fails!!!
  w.briefsblack.loc = "lounge"
  w.halterblack.loc = "lounge"
  ...
  test.assertEqual(true, w.Clive.hasBodyPart('chest'))
  test.assertEqual(true, w.Clive.hasBodyPart('cock'))
  test.assertEqual(false, w.Clive.hasBodyPart('tit'))
  test.assertEqual(false, w.Clive.hasBodyPart('wing'))
  test.assertEqual(true, w.Joanna.hasBodyPart('chest'))
  test.assertEqual(true, w.Joanna.hasBodyPart('tit'))
  test.assertEqual(false, w.Joanna.hasBodyPart('cock'))


  test.title("reveal")
  w.Joanna.dressUp("teeshirtmesh", "halterblack", "briefsblack")
  test.assertEqual(1, findBodyPart('midriff').getAllCoverings(w.Joanna).length)
  test.assertEqual(2, findBodyPart('chest').getAllCoverings(w.Joanna).length)
  test.assertEqual(3, findBodyPart('midriff').getReveal(w.Joanna))
  test.assertEqual(false, findBodyPart('chest').getReveal(w.Joanna))
  w.Joanna.dressUp("halterblack", "briefsblack")
  ...
  w.Joanna.dressUp("jeans2", "teeshirtwhite", "halterblack", "briefsblack", "heels")
  test.assertEqual(1, findBodyPart('foot').getAllCoverings(w.Joanna).length)
  test.assertEqual(2, w.Joanna.getExposure())
  w.Joanna.dressUp("briefsblack")
  ...
  /*
  rating in commandsX.js l ca 890 is far too low
  ...
  */
  ...
  test.assertCmd("wank j", "Joanna doesn't have a cock.")
  ...
  test.assertCmd("j,undress", ["'Sure thing!'", "Joanna pulls the white teeshirt up and over her head."])
  ...
  world.update()
  test.assertCmd("remove ha from j", ["You unfasten Joanna's black bikini halter, pulling it off her."])
  ...
  w.briefsblack.worn = false
  test.assertCmd("j, fuck me", ["Joanna bounces up and down on your cock for a few minutes."])
...
log('-----')
  test.assertCmd("pour honey over joanna's thighs", ["You pour honey on to Joanna's thighs."])
log('-----')
  ...
  delete w[g3.name]
  g1.variFunc = function(options) {
    this.alias = options.color + " panties"
  }
